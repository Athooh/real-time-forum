...
import { createComment } from "../components/posts/postsTemplates.js";
...
export function handleNewReply(payload) {
  const { commentId, reply } = payload;
  const repliesContainer = document.querySelector(`#replies-${commentId}`);

  if (repliesContainer) {
    const replyHTML = createComment(reply, true, false);
    repliesContainer.insertAdjacentHTML("beforeend", replyHTML);
  }
}

export function handleCommentCountUpdate(data) {
