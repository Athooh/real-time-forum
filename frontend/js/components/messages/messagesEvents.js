...
export async function handleChatOpen(userId) {
  try {
    ...
    // Mark the conversation as read if it has unread messages
    if (activeItem && activeItem.classList.contains("unread-message")) {
      // Remove unread styling
      activeItem.classList.remove("unread-message");
      ...
      const messagePreview = activeItem.querySelector(".message-preview");
      if (messagePreview) {
    ...
    await markVisibleMessagesAsRead(userId);
    ...
    if (
      ...
      try {
        ...
        if (olderMessages && olderMessages.length > 0) {
          ...
          // Mark newly loaded unread messages as read
          const currentUserId = getCurrentUserId();
          const unreadMessages = olderMessages.filter(
            (msg) =>
          ...
          if (unreadMessages.length > 0) {
            const markReadPromises = unreadMessages.map((msg) => {
              const msgElement = chatMessages.querySelector(
            ...
            // Process these in the background
            Promise.all(markReadPromises).catch((error) => {
              console.error("Error marking scrolled messages as read:", error);
            });
          }
...
function createMessageElement(message) {
  ...
  const isRead = message.is_read || isOwn; // Messages sent by current user are always "read"
...
// Add this function to mark messages as read when they're displayed
export async function markVisibleMessagesAsRead(userId) {
  try {
    const chatMessages = document.querySelector(".chat-messages");
    if (!chatMessages) return;

    // Find all unread messages from this user
    const unreadMessages = chatMessages.querySelectorAll(
      `.chat-message.received[data-sender-id="${userId}"]:not(.read)`
    );
    ...
    const markReadPromises = Array.from(unreadMessages).map((msgElement) => {
      const messageId = msgElement.dataset.messageId;
      // Add read class to the message
      msgElement.classList.add("read");
    ...
    // Update the message list item to remove unread status
